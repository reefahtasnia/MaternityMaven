...
const dbConnection = require("./"); // Your database connection file

// Schedule a job to run every minute
cron.schedule("* * * * *", async () => {
  let conn;
  try {
    conn = await dbConnection(); // Get your database connection

    // Fetch users and their prescriptions in one query
    const query = `
      SELECT u.userId, u.email, m.medicine_name, m.dosage, m.time 
      FROM users u 
      JOIN medicinetracker m ON u.userId = m.userId`;
